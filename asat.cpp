...
void find_unsatisfied_clauses(const sat_prob &A, const std::vector<int> &conf, std::vector<int> &R, std::vector<int> &W){
...
std::vector<int> solve_asat (sat_prob &A, double p){
	...
	for(size_t t = 0; t<t_max;++t){
		...
		if(configuration[abs(A.get_clause(w).v[z])-1] == 0)	
			configuration[abs(A.get_clause(w).v[z])-1] = 1;
		else
			configuration[abs(A.get_clause(w).v[z])-1] = 0;
			
			
		find_unsatisfied_clauses(A,configuration,R,CFx[abs(A.get_clause(w).v[z])-1]);
